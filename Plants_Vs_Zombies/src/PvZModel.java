...
public class PvZModel {
	...
	Stack<ArrayList<ArrayList<GridCell>>> undo, redo, save;
	Stack<Integer> undoSunlight, redoSunlight, saveSunlight;
	...
	public PvZModel(int i, int j)
	{
		...
		save = new Stack<ArrayList<ArrayList<GridCell>>>();
		...
		saveSunlight = new Stack<Integer>();
	...
	public void createCopy(ArrayList<ArrayList<GridCell>> a) {
		...
		for (int r = 0; r < 5; r++) {
			...
			for (int c = 0; c < 8; c++) {
				...
				//checks what type of plant is in the cell
				...
				//checks if the zombie is a regular or boss zombie
	...
	public void placePlantAt(int x, int y)
	{
		...
		if (buttonClicked.equals("S")) {
			...
			//do nothing if a plant has not been clicked yet
		}
		numMoves++;
	}
	...
	public void placeSunflowerAt(int x, int y)
	{
		if (sunlight <50)
			...
			placePlantHelper(); //plant helper spawns a zombie in a random row and updates sunlight
		}	
